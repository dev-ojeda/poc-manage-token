...
export async function loadActiveSessions(api_admin, reloadFn) {
    ...
    try {
        ...
        const res = await api_admin.loadActiveSessions(payload);
        ...
        res.sessions.forEach(session => {
            ...
            row.innerHTML = `
                <td>${session.user_data.username}</td>
...
function showTableSpinner() {
    const tbody = document.querySelector("#tablaSesiones tbody");
    const columnCount = document.querySelectorAll("#tablaSesiones thead th").length || 8;
    if (!tbody) return;
    tbody.innerHTML = `
        <tr>
